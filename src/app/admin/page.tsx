...
export default async function AdminDashboard() {
 ...
 <div>
 <h1 className="text-2xl font-black text-gray-800 tracking-tighter mb-2 ">Mission Control</h1>
 <p className="text-gray-500 font-medium uppercase tracking-wider text-sm">System Status: <span className="text-emerald-600 font-bold glow-green">Nominal</span></p>
 </div>
 <div className="text-right hidden md:block">
 <p className="text-sm font-black text-primary uppercase tracking-[0.5em] mb-2">Current Cycle</p>
 <p className="text-2xl font-bold text-gray-800 font-mono">{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
 </div>
 </div>
 ...
 <h2 className="text-3xl font-black text-gray-800 tracking-tight">Direct Commands</h2>
 </div>

 <div className="grid gap-6">
 <Link href="/admin/bundles/new" className="group/item flex items-center justify-between p-6 bg-white rounded-lg hover:bg-primary hover:border-primary border border-gray-200 shadow-sm transition-all duration-300">
 ...
 <span className="font-bold text-lg text-gray-600 group-hover/item:text-gray-800">Initialize Bundle</span>
 </div>
 <ArrowRight size={24} className="text-slate-300 group-hover/item:text-gray-800 -translate-x-4 group-hover/item:translate-x-0 transition-all" />
 </Link>

 <Link href="/admin/enquiries" className="group/item flex items-center justify-between p-6 bg-white rounded-lg hover:bg-white hover:text-gray-800 border border-gray-200 shadow-sm transition-all duration-300">
 <div className="flex items-center gap-6">
 ...
 <span className="font-bold text-lg text-gray-600 group-hover/item:text-gray-800">Process Signals</span>
 </div>
 <ArrowRight size={24} className="text-slate-300 group-hover/item:text-gray-800 -translate-x-4 group-hover/item:translate-x-0 transition-all" />
 </Link>
 </div>
 ...
 <h2 className="text-3xl font-black text-gray-800 tracking-tight">Neural Network</h2>
 </div>
...
function StatusRow({ label, status, color }: any) {
 ...
 <span className="text-gray-500 font-bold tracking-wide group-hover:text-gray-800 transition-colors">{label}</span>
 <span className={`flex items-center gap-3 ${color} font-black text-sm uppercase tracking-wider`}>
 <div className={`w-2 h-2 rounded-lg currentColor shadow-[0_0_10px_currentColor] animate-pulse`} style={{ backgroundColor: 'currentColor' }} />
 {status}
 </span>
 </div>
 )
...
function StatsCard({ title, value, icon, trend, color, isStatus }: any) {
 ...
 return (
 <div className={`bg-white rounded-lg p-8 shadow-xl border border-gray-200 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300 hover:shadow-2xl`}>
 {isStatus && value > 0 && (
 ...
 <div className={`p-4 rounded-lg bg-slate-50 border border-gray-200 text-gray-800 group-hover:scale-110 transition-transform duration-500 group-hover:bg-white group-hover:shadow-md`}>
 {icon}
 </div>
 {isStatus && value > 0 && (
 ...
 <span className="text-sm font-black text-red-600 uppercase tracking-wider">Alert</span>
 </div>
 )}
 </div>

 <div>
 <h3 className="text-gray-500 text-sm font-black uppercase tracking-wider mb-2 group-hover:text-primary transition-colors">{title}</h3>
 <div className="flex items-baseline gap-4">
 <p className={`text-2xl font-black tracking-tighter text-gray-800`}>{value}</p>
 <span className="text-xs font-bold text-gray-500 group-hover:text-gray-600 transition-colors">{trend}</span>
 </div>
 </div>
 </div>

 {/* Holographic Border Effect */}
 <div className={`absolute inset-0 border-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none ${activeColor.split(' ')[1]}`} />
 </div>
 );
}
