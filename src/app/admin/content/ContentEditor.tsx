...
import { Edit2, Save, X, FileText, CheckCircle2, Clock, Search, Layout } from 'lucide-react';
...
export default function ContentEditor({ initialContent }: ContentEditorProps) {
 ...
 <button
 onClick={() => alert('Client JavaScript is working!')}
 className="px-4 py-2 bg-red-500 text-gray-800 rounded font-bold"
 >
 TEST: Click Me
 </button>

 {/* Search and Filters Bar */}
 <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm mb-8 flex items-center gap-4">
 <div className="relative flex-1">
 <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={18} />
 <input
 type="text"
 placeholder="Search content by key or text..."
 className="w-full pl-12 pr-4 py-2.5 bg-slate-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium"
 value={searchTerm}
 ...
 <button key={page} className="px-4 py-2 bg-slate-50 border border-gray-200 rounded-lg text-xs font-bold uppercase tracking-wider text-gray-500 hover:bg-slate-100 transition-colors">
 {page}
 </button>
 ))}
 </div>
 ...
 <div>
 <h2 className="text-xl font-extrabold text-gray-800 tracking-tight">{page} Page</h2>
 <p className="text-xs text-gray-500 font-medium">{filteredItems.length} Editable Fields</p>
 </div>
 </div>

 <div className="grid gap-6">
 {filteredItems.map(item => (
 <div key={item.key} className={`bg-white rounded-lg border transition-all duration-300 ${editingKey === item.key ? 'border-primary shadow-xl ring-4 ring-primary/5' : 'border-gray-200 shadow-sm hover:shadow-md hover:border-gray-200'}`}>
 <div className="p-6">
 ...
 <Clock size={12} />
 Last updated {new Date(item.updatedAt).toISOString().split('T')[0]}
 </div>
 </div>
 ...
 {item.content || <em className="text-gray-500">No content provided</em>}
 </div>
 <button
