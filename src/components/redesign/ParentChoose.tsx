...
const ParentChoose = ({ content = {} }: ParentChooseProps) => {
  const getContent = (key: string, fallback: string = "") => {
    return content[key] || fallback;
  };

  const reasons = [
    { title: getContent("reason1_title", "Neuroscience-Backed"), icon: Brain, desc: getContent("reason1_desc", "Revisiting concepts with progressive complexity."), color: "var(--brand-purple-deep)" },
    { title: getContent("reason2_title", "Crafted with Love"), icon: Heart, desc: getContent("reason2_desc", "Built by educators who believe every child deserves to experience the joy of truly understanding something."), color: "var(--brand-gold)" },
    { title: getContent("reason3_title", "Globally Relevant"), icon: Globe2, desc: getContent("reason3_desc", "Designed for the international education market — timeless enough for today, visionary enough for tomorrow."), color: "var(--brand-blue)" },
  ];
